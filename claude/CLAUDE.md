...
- Plans are temporary artifacts, not documentation: a written plan lives at `<repo root>/.claude/plans/<feature>.md`, committed, with `status:` frontmatter. The moment its last phase/PR lands, set `status: shipped` — that arms the pre-push gate — then route anything durable out (my:capture-learnings) and delete the file. Never archive a plan; git history is the archive.
