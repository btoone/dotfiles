...
description: >
  The depth behind the coding-philosophy rules in the global CLAUDE.md —
  rationale, worked examples, and reference material for design-tradeoff
  moments. Covers DDD building blocks (Evans), the declarative-domain /
  imperative-orchestration split with a worked Ruby example, the "-er" naming
  heuristic, SOLID with Ruby pragmatism, simplicity rules (Rule of Three,
  patterns-when-earned, optimize-with-evidence), CD/DORA, Pragmatic Programmer
  principles, the Paul Hammond testing worldview (fakes over mocks, mutation
  testing, test-evidence ownership), and the Influences table. Do NOT use for
  mechanical edits, non-code tasks, or work already covered by a more specific
  skill (my:tdd for the TDD cycle, my:bounded-contexts for context placement,
  my:comments for comment style).
when_to_use: >
  Use when making a design tradeoff: structuring or naming new code, choosing
  between abstractions, deciding where behavior lives, reviewing a design or
  PR for architecture, or when the user asks "how should I model this",
  "what would you call this", or invokes the coding philosophy. If the terse
  rule in the global CLAUDE.md is ambiguous for the case at hand, this skill
  holds the reasoning to resolve it.
---

# Design Philosophy — the depth behind the rules

The always-on rules live in the global CLAUDE.md (one line each). This skill
holds the rationale, worked examples, and reference material behind them.
...
- **Bounded Contexts** — A `Transaction` in the Visa Draft 256 context is not the same as a `Transaction` in the API context. Respect these boundaries rather than forcing a single model to serve every context. (Placement decisions: use the my:bounded-contexts skill.)
...
The TDD cycle (red/green/refactor) is non-negotiable — it's a design tool, not just a verification tool. (The working cycle lives in the my:tdd skill.)
...
### Manage dependencies at the boundary
...
### Name with nouns; treat an "-er" as a prompt, not a verdict

Agent-noun classes coined from verbs (`Materializer`, `Resolver`, `Manager`, `Processor`, `Handler`) are often a smell — a function wishing it were a function, behavior stolen from an entity (Fowler's Anemic Domain Model), or a missing noun. When I reach for one I ask: does this have a cohesive identity beyond a single `call`, or is it hiding a noun — or behavior an entity should own? Default to domain nouns for value objects and services (`RateSchedule`, `EligibilityCheck`, `BookingEstimate` — not `…Checker`/`…Estimator`), and prefer moving record-level predicates onto the record (a most-specific-wins lookup is usually a finder on the aggregate, e.g. `Plan.in_force_for(...)`, not a `PlanResolver`). This is *not* absolute: Evans' Domain Services are legitimately named for activities when an operation spans aggregates and belongs to no entity, so a real boundary operation — an `Ingestor`, a `Translator` at an anti-corruption layer — earns its "-er." The heuristic raises the question; it doesn't mandate the rename.

## Testing Worldview (Paul Hammond)

XP-lineage testing practices distilled from Paul Hammond (Pack Software; `citypaul` on GitHub). They compose with the BDD conventions above — BDD supplies the spec *language*, this supplies the evidence *placement and strength*.

- **Tests are executable specifications.** They describe what the system does for a consumer, in business language, at a public boundary. "TDD is not about design, TDD is about feedback" — a complementary framing to Beck's design-tool view; hold both. The payoff is confidence: if tests pass, you should feel able to ship.
- **The claim under test decides which boundary owns the evidence.** Every layer has its own public interface. A "user creates X" test that calls the HTTP endpoint directly proves an HTTP contract, not a UI flow. Test names claim only the narrowest evidence actually proved.
- **Don't test what another layer owns.** Test boundaries follow architectural boundaries; business rules may be deliberately duplicated at domain and API layers as defense in depth — which test fails tells you which boundary broke.
- **Fakes over mocks** (Meszaros sense). A fake implements the real interface, so the compiler/contract breaks it when the interface changes, and it verifies state ("was the data saved?") not interaction ("was `.save()` called?"). Spying on internal collaborators is the anti-pattern — mock-heavy tests stay green while the product promise breaks. Nuance: asserting on a callback the caller provided through the public surface (an `onSubmit` prop) is behavior, not spying.
- **Mock at the network edge if at all** — one mental model at every layer; unhandled requests fail the test so URL drift is caught. Network-level handlers can assert the outgoing request shape, so no spy on the client is needed.
- **Test data via factories** — complete valid objects with per-test overrides, validated by the real schema; no `let` + `beforeEach` shared state. Composition factories supply a fixed clock and silent fakes.
- **Effects are ports.** Inject the clock (`now` as a dependency) instead of faking timers — time is an external effect. Introduce a port only when behavior must swap in tests or plausibly in production; never for internal helpers. A port is only real if it is tested: every driven port gets a fake, every driving port a test driver.
- **Coverage measures execution; mutation testing measures detection.** 100% coverage can still miss a large share of bugs. Mutation-aware example selection happens at RED time: "if my implementation were subtly wrong, which example would catch it?" (boundary values, near-misses). Adoption ladder: manually break code during PR review → incremental mutation runs → diff-scoped PR gate → thresholds only after a baseline exists.
...
| **Paul Hammond** | XP testing practice — executable specifications at public boundaries, fakes over mocks, mutation testing, effects as ports |
